...
const ProductForm = () => {
    const [payload, setPayload] = useState<IPayload>({
      ...
      const handleSubmit = (e: FormEvent) => {
        ...
        .then((res) => {
            makeToast("Produk telah ditambahkan!");
            setPayload({
                imgSrc: null,
                fileKey: null,
                name: "",
  ...
  return(
    <form className='flex flex-col gap-4' onSubmit={handleSubmit}>
        <Image className='max-h-[300px] w-auto object-contain rounded-md' src={payload.imgSrc ? payload.imgSrc : "/photos.png"}
        width={800}
        height={500}
        alt='product_image'/>
        ...
        <div className='flex justify-end'>
            <button className='bg-blue-700 text-white px-8 py-2 rounded-md'>UPLOAD</button>
        </div>
    </form>
  )
